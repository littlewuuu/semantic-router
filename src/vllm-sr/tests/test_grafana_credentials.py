...
def test_fresh_stack_generates_and_reuses_a_container_readable_password_file(
    ...
    assert path.exists()
    # The file is container-readable on purpose: Grafana runs unprivileged and
    # must read the bind-mounted value. Privacy comes from the 0700 parent dir,
    # which the second resolve also must not have revoked by re-hardening.
...
def test_explicit_env_password_is_materialized_for_the_container(
    ...
    assert _file_mode(active_path.parent) == 0o700
    assert _file_mode(active_path) == 0o644
...
def test_unsetting_explicit_env_falls_back_to_a_new_generated_secret(
    ...
    assert _file_mode(persisted_path) == 0o644
...
def test_grafana_container_reads_password_from_secret_file_never_argv(
    ...
    # The mounted source must stay readable by the unprivileged Grafana uid
    # even after the CLI read the password back for its own use.
...
def test_grafana_container_mounts_the_explicit_password_file(monkeypatch, tmp_path):
    ...
    assert _file_mode(explicit_path) == 0o644
    assert (
        f"{gc.GRAFANA_ADMIN_PASSWORD_FILE_ENV}=" f"{gc.CONTAINER_GRAFANA_PASSWORD_PATH}"
    ) in command
